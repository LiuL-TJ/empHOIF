## The following code is the key part of computing the three different versions of IF22 when the nuisance working models are linear models
## Input: Highdim_Nuisance_Ln_*.RData, including the nuisance function estimators using linear models and hat(g) using kde
...
for (i in 1:d) {
    ...
    PS <- cbind(PS, as.numeric(sapply(1:grps, function (j) Sim.Data[[i + (j - 1) * d]][[4]]))[1:n])
    OR <- cbind(OR, as.numeric(sapply(1:grps, function (j) Sim.Data[[i + (j - 1) * d]][[5]]))[1:n])
}
...
Data$PS_est <- predict(PSmod, newdata = Data, type = 'response')
Data$OR_est <- predict(ORmod, newdata = Data)
Data$PS_res <- Data$Y - Data$PS_est
Data$OR_res <- Data$A - Data$OR_est
...
library(ks)
Data$ghat <- predict(ghat, x = Data[, 1:d])
left.basis <- DB / Data$ghat^(0.5)
...
OmegaA <- OmegaA * rr
OmegaAAC <- OmegaA[filter.cols, filter.cols] * mean(Data$A)
...
save(psi.mar, IF1.mar.mis, IF1.mar.null, IF22.mar.emp, IF22.mar.emp.null, IF22.mar.ker, IF22.mar.ac, file = paste0('IF22_Highdim_Ln_', new.seed, '_', beta.b, '_', beta.p, '_', beta.g, '_', Rb, '_', Rp, '_', Rg, '_', varY, '_', filter.number, '_', filter.number.analysis, '_', ntr, '_', d, '_', n, '_', k.n, '.RData'))
